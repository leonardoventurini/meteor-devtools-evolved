...
const VALUE_OPERATORS = {
  ...
  $near(operand, valueSelector, matcher, isRoot) {
    ...
    if (
      LocalCollection._isPlainObject(operand) &&
      hasOwn.call(operand, '$geometry')
    ) {
...
export function makeLookupFunction(key, options = {}) {
  ...
  const lookupRest =
    parts.length > 1 && makeLookupFunction(parts.slice(1).join('.'), options)
...
function operatorBranchedMatcher(valueSelector, matcher, isRoot) {
  ...
  const operatorMatchers = Object.keys(valueSelector).map(operator => {
    ...
    const simpleEquality =
      ['$ne', '$eq'].includes(operator) && operand !== Object(operand)
