...
Tinytest.add('minimongo - basics', test => {
  ...
  test.equal(
    c.find({ _id: 1 }, { sort: ['_id', 'desc'], skip: 1 }).count(false),
    1,
  )
  ...
  test.equal(
    c.find({}, { sort: ['_id', 'desc'], skip: 1, limit: 1 }).count(false),
    3,
  )
  test.equal(
    c.find({}, { sort: ['_id', 'desc'], skip: 1, limit: 1 }).count(),
    1,
  )
  test.equal(
    c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1 }).count(false),
    2,
  )
  test.equal(
    c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1 }).count(),
    1,
  )
  test.equal(
    c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], limit: 1 }).count(false),
    2,
  )
  test.equal(
    c.find({ tags: 'fruit' }, { sort: ['_id', 'desc'], limit: 1 }).count(),
    1,
  )
  test.equal(
    c
      .find({ tags: 'fruit' }, { sort: ['_id', 'desc'], skip: 1, limit: 1 })
      .count(false),
    2,
  )
  test.equal(
...
Tinytest.add('minimongo - selector_compiler', test => {
  const matches = (shouldMatch, selector, doc) => {
    const doesMatch = new Minimongo.Matcher(selector).documentMatches(
      doc,
    ).result
  ...
  ;[5, [10], [10, 1, 2], 'foo', { bar: 1 }, []].forEach(badMod => {
    test.throws(() => {
  ...
  // $bitsAllClear - buffer
  match(
    { a: { $bitsAllClear: new Uint8Array([3]) } },
    { a: new Uint8Array([4]) },
  )
  match(
    { a: { $bitsAllClear: new Uint8Array([0, 1]) } },
  ...
  match(
    { a: { $bitsAnySet: new Uint8Array([15]) } },
    { a: new Uint8Array([7]) },
  )
  ...
  // $bitsAnyClear - buffer
  match(
    { a: { $bitsAnyClear: new Uint8Array([8]) } },
    { a: new Uint8Array([7]) },
  )
  match(
    { a: { $bitsAnyClear: new Uint8Array([1]) } },
    { a: new Uint8Array([0]) },
  )
  ...
  matchCount(
    {
      a: {
        $bitsAllClear: EJSON.parse(
          '{"$binary": "AAAAAAAAAAAAAAAAAAAAAAAAAAAA"}',
        ),
      },
    },
    4,
  ...
  matchCount(
    {
      a: {
        $bitsAllClear: EJSON.parse(
          '{"$binary": "AAyfAAAAAAAAAAAAAAAAAAAAAAAA"}',
        ),
      },
    },
    3,
  ...
  matchCount(
    {
      a: {
        $bitsAllClear: EJSON.parse(
          '{"$binary": "JAyfqwetkqwklEWRbWERKKJREtbq"}',
        ),
      },
    },
    2,
  ...
  matchCount(
    {
      a: {
        $bitsAnyClear: EJSON.parse(
          '{"$binary": "AANgAAAAAAAAAAAAAAAAAAAAAAAA"}',
        ),
      },
    },
    1,
  ...
  matchCount(
    {
      a: {
        $bitsAnyClear: EJSON.parse(
          '{"$binary": "JANgqwetkqwklEWRbWERKKJREtbq"}',
        ),
      },
    },
    2,
  ...
  match(
    { $or: [{ a: { $in: [1, 2, 3] } }, { b: { $in: [1, 2, 3] } }] },
    { b: 2 },
  )
  nomatch(
    { $or: [{ a: { $in: [1, 2, 3] } }, { b: { $in: [4, 5, 6] } }] },
    { b: 2 },
  )
  ...
  match(
    { $or: [{ a: { b: 1, c: 2 } }, { a: { b: 2, c: 1 } }] },
    { a: { b: 1, c: 2 } },
  )
  nomatch(
    { $or: [{ a: { b: 1, c: 3 } }, { a: { b: 2, c: 1 } }] },
    { a: { b: 1, c: 2 } },
  )
  ...
  nomatch(
    { $nor: [{ a: { $in: [1, 2, 3] } }, { b: { $in: [1, 2, 3] } }] },
    { b: 2 },
  )
  match(
    { $nor: [{ a: { $in: [1, 2, 3] } }, { b: { $in: [4, 5, 6] } }] },
    { b: 2 },
  )
  ...
  nomatch(
    { $nor: [{ a: { b: 1, c: 2 } }, { a: { b: 2, c: 1 } }] },
    { a: { b: 1, c: 2 } },
  )
  match(
    { $nor: [{ a: { b: 1, c: 3 } }, { a: { b: 2, c: 1 } }] },
    { a: { b: 1, c: 2 } },
  )
  ...
  nomatch(
    { $and: [{ a: { $in: [1, 2, 3] } }, { a: { $in: [4, 5, 6] } }] },
    { a: 1 },
  )
  nomatch(
    { $and: [{ a: { $in: [1, 2, 3] } }, { b: { $in: [1, 2, 3] } }] },
    { a: 1, b: 4 },
  )
  match(
    { $and: [{ a: { $in: [1, 2, 3] } }, { b: { $in: [4, 5, 6] } }] },
    { a: 1, b: 4 },
  )
  ...
  nomatch(
    { $and: [{ a: { $nin: [1, 2, 3] } }, { a: { $nin: [4, 5, 6] } }] },
    { a: 1 },
  )
  nomatch(
    { $and: [{ a: { $nin: [1, 2, 3] } }, { b: { $nin: [1, 2, 3] } }] },
    { a: 1, b: 4 },
  )
  nomatch(
    { $and: [{ a: { $nin: [1, 2, 3] } }, { b: { $nin: [4, 5, 6] } }] },
    { a: 1, b: 4 },
  )
  ...
  match(
    { $and: [{ a: { $not: { $lt: 0 } } }, { a: { $not: { $gt: 2 } } }] },
    { a: 1 },
  )
  nomatch(
    { $and: [{ a: { $not: { $lt: 2 } } }, { a: { $not: { $gt: 0 } } }] },
    { a: 1 },
  )
  ...
  match(
    { 'animals.dogs.name': 'Fido' },
    {
      animals: [{ dogs: { name: 'Rex' } }, { dogs: { name: 'Fido' } }],
    },
  )
  match(
    { 'animals.dogs.name': 'Fido' },
...
Tinytest.add('minimongo - projection_compiler', test => {
  ...
  testProjection({ _id: 1 }, [
    [{ _id: 42, x: 1, y: { z: '2' } }, { _id: 42 }, '_id whitelisted'],
    [{ _id: 33 }, { _id: 33 }, '_id whitelisted, _id only'],
    [{ x: 1 }, {}, '_id whitelisted, no _id'],
  ])
  ...
  testProjection({ _id: 0 }, [
    ...
    [{ _id: 33 }, {}, '_id blacklisted, _id only'],
    [{ x: 1 }, { x: 1 }, '_id blacklisted, no _id'],
  ])

  testProjection({}, [
    [{ a: 1, b: 2, c: '3' }, { a: 1, b: 2, c: '3' }, 'empty projection'],
  ])
...
Tinytest.add('minimongo - fetch with fields', test => {
  ...
  test.isTrue(
    fetchResults.every(x => x && x.something && x.i >= 10 && x.i < 20),
  )
...
Tinytest.add('minimongo - fetch with projection, subarrays', test => {
  ...
  const testForProjection = (projection, expected) => {
    ...
    equalNonStrict(
      fetched,
      expected,
      `failed sub-set projection: ${JSON.stringify(projection)}`,
    )
  }
...
Tinytest.add('minimongo - fetch with projection, deep copy', test => {
  ...
  test.equal(
    filteredDoc.b.y.z,
    33,
    'projection returning deep copy - including',
  )
...
Tinytest.add('minimongo - observe ordered with projection', test => {
  ...
  handle = c
    .find({}, { sort: { a: -1 }, fields: { a: 1 } })
    .observe(Object.assign(cbs, { _suppress_initial: true }))
  test.equal(operations.shift(), undefined)
  ...
  c.remove({})
  handle = c
    .find({}, { sort: { a: 1 }, skip: 1, limit: 2, fields: { blacklisted: 0 } })
    .observe(cbs)
  test.equal(operations.shift(), undefined)
  ...
  handle = c
    .find({}, { sort: { a: 1 }, fields: { a: 1 } })
    .observe(Object.assign(cbs, { _no_indices: true }))
...
Tinytest.add('minimongo - ordering', test => {
  ...
  verify({ 'a.0': 1 }, [{ a: [1, [10, 15], 20] }, { a: [5, [-5, -20], 18] }])
  verify({ 'a.0': -1 }, [{ a: [5, [-5, -20], 18] }, { a: [1, [10, 15], 20] }])
  // Similarly, this is just comparing [-5,-20] to [10, 15].
  verify({ 'a.1': 1 }, [{ a: [5, [-5, -20], 18] }, { a: [1, [10, 15], 20] }])
  verify({ 'a.1': -1 }, [{ a: [1, [10, 15], 20] }, { a: [5, [-5, -20], 18] }])
  ...
  verify({ 'a.1': 1 }, [{ a: [1, [10, 15], 20] }, { a: [5, [19, 3], 18] }])
  verify({ 'a.1': -1 }, [{ a: [5, [19, 3], 18] }, { a: [1, [10, 15], 20] }])
  // Minimal elements are 1 and 5.
  verify({ a: 1 }, [{ a: [1, [10, 15], 20] }, { a: [5, [19, 3], 18] }])
  // Maximal elements are [19,3] and [10,15] (because arrays sort higher than
  // numbers), even though there's a 20 floating around.
  verify({ a: -1 }, [{ a: [5, [19, 3], 18] }, { a: [1, [10, 15], 20] }])
  ...
  verify({ a: -1 }, [{ a: [1, [10, 15], 20] }, { a: [5, [3, 19], 18] }])
...
Tinytest.add('minimongo - sort', test => {
  ...
  test.equal(
    c
      .find({ a: { $gt: 10 } }, { sort: { b: -1, a: 1 }, skip: 3, limit: 5 })
      .fetch(),
    [
      { a: 14, b: 1, _id: '14_1' },
      { a: 15, b: 1, _id: '15_1' },
  ...
  test.equal(
    c
      .find({ a: { $gte: 20 } }, { sort: { a: 1, b: -1 }, skip: 50, limit: 5 })
      .fetch(),
    [
      { a: 45, b: 1, _id: '45_1' },
      { a: 45, b: 0, _id: '45_0' },
...
Tinytest.add('minimongo - nested array sort', test => {
  ...
  testCursorMatchesField(
    c.find({}, { sort: { 'c.d.x': 1, 'c.d.y': 1 } }),
    'cdx_cdy',
  )
  testCursorMatchesField(
    c.find({}, { sort: { 'c.d.y': 1, 'c.d.x': 1 } }),
    'cdy_cdx',
  )
})
...
Tinytest.add('minimongo - sort keys', test => {
  ...
  testKeys({ 'a.x': 1, 'a.y': 1 }, { a: { x: 0, y: 5 } }, [[0, 5]])
  ...
  testKeys({ a: 1, b: 1 }, { a: [1, 2, 3], b: 42 }, [
    [1, 42],
    [2, 42],
    [3, 42],
  ])

  testKeys({ 'a.0.x': 1 }, { a: [{ x: 0 }] }, [[0]])

  testKeys({ 'a.0.x': 1 }, { a: [] }, [[undefined]])

  // Don't support multiple arrays at the same level.
  testParallelError({ a: 1, b: 1 }, { a: [1, 2, 3], b: [42] })
...
Tinytest.add('minimongo - sort function', test => {
  ...
  test.equal(
    c.find({}, { sort: sortFunction }).fetch(),
    c.find({}).fetch().sort(sortFunction),
  )
  test.notEqual(c.find({}).fetch(), c.find({}).fetch().sort(sortFunction))
  test.equal(
    c.find({}, { sort: { a: -1 } }).fetch(),
    c.find({}).fetch().sort(sortFunction),
  )
})
...
Tinytest.add('minimongo - binary search', test => {
  ...
  checkSearchForward(
    ...
    1,
    3,
    'Highly degenerate array, lower',
  )
  checkSearchForward(
    [1, 1, 1, 2, 2, 2, 2],
    2,
    7,
    'Highly degenerate array, upper',
  )
  checkSearchForward(
    [2, 2, 2, 2, 2, 2, 2],
    1,
  ...
  checkSearchBackward(
    ...
    1,
    7,
    'Backward: Degenerate array, lower',
  )
  checkSearchBackward(
    [2, 2, 2, 2, 1, 1, 1],
    2,
    4,
    'Backward: Degenerate array, upper',
  )
  checkSearchBackward(
    [2, 2, 2, 2, 2, 2, 2],
    1,
    7,
    'Backward: Highly degenerate array, upper',
  )
  checkSearchBackward(
    [2, 2, 2, 2, 2, 2, 2],
    2,
...
Tinytest.add('minimongo - modify', test => {
  ...
  // $currentDate
  modify({}, { $currentDate: { a: true } }, (result, msg) => {
    test.instanceOf(result.a, Date, msg)
  })
  modify({}, { $currentDate: { a: { $type: 'date' } } }, (result, msg) => {
    test.instanceOf(result.a, Date, msg)
  })
  ...
  modify({}, { $push: { a: { $each: [1, 2, 3] } } }, { a: [1, 2, 3] })
  modify({ a: [] }, { $push: { a: { $each: [1, 2, 3] } } }, { a: [1, 2, 3] })
  modify(
    { a: [true] },
    { $push: { a: { $each: [1, 2, 3] } } },
    { a: [true, 1, 2, 3] },
  ...
  modify(
    { a: [1, 2, 3] },
    { $push: { a: { $each: [4, 5], $slice: 1 } } },
    { a: [1] },
  )
  modify(
    { a: [1, 2, 3] },
    { $push: { a: { $each: [4, 5], $slice: 2 } } },
    { a: [1, 2] },
  )
  modify(
    { a: [1, 2, 3] },
    { $push: { a: { $each: [4, 5], $slice: 4 } } },
    { a: [1, 2, 3, 4] },
  )
  modify(
    { a: [1, 2, 3] },
    { $push: { a: { $each: [4, 5], $slice: 5 } } },
    { a: [1, 2, 3, 4, 5] },
  )
  modify(
    { a: [1, 2, 3] },
    { $push: { a: { $each: [4, 5], $slice: 10 } } },
    { a: [1, 2, 3, 4, 5] },
  )
  ...
  modify({ a: [] }, { $pull: { a: 3 } }, { a: [] })
  modify({ a: [[2], [2, 1], [3]] }, { $pull: { a: [2, 1] } }, { a: [[2], [3]] }) // tested
  modify(
    {
      a: [
        { b: 1, c: 2 },
  ...
  upsert({ a: { $exists: true } }, { $setOnInsert: { a: 123 } }, { a: 123 })
  ...
  upsert({ a: { $eq: testObjectId } }, { $setOnInsert: { a: 123 } }, { a: 123 })
...
Tinytest.add('minimongo - objectid transformation', test => {
  ...
  test.equal(
    'ffffffffffff',
    MongoID.idParse(MongoID.idStringify('ffffffffffff')),
  )
})
...
Tinytest.add('minimongo - $near operator tests', test => {
  ...
  test.equal(
    coll.find({ 'rest.loc': { $near: [0, 0], $maxDistance: 30 } }).count(),
    3,
  )
  test.equal(
    coll.find({ 'rest.loc': { $near: [0, 0], $maxDistance: 4 } }).count(),
    1,
  )
  const points = coll
    .find({ 'rest.loc': { $near: [0, 0], $maxDistance: 6 } })
    .fetch()
  points.forEach((point, i, points) => {
    test.isTrue(
      !i ||
        distance([0, 0], point.rest.loc) >=
          distance([0, 0], points[i - 1].rest.loc),
    )
  })
  ...
  data.forEach((x, i) => {
    coll.insert(Object.assign(x, { x: i }))
  })
  ...
  const testNear = (near, md, expected) => {
    test.equal(
      coll
        .find({ 'a.b': { $near: near, $maxDistance: md } })
        .fetch()
        .map(doc => doc._id),
      expected,
    )
