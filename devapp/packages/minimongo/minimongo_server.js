...
Minimongo._pathsElidingNumericKeys = paths =>
  paths.map(path =>
    path
      .split('.')
      .filter(part => !isNumericKey(part))
      .join('.'),
  )
...
Minimongo.Matcher.prototype.canBecomeTrueByModifier = function (modifier) {
  ...
  if (
    this._getPaths().some(pathHasNumericKeys) ||
    ...
    path => {
      ...
      if (isOperatorObject(valueSelector)) {
        ...
        if (valueSelector.$in) {
          ...
          return valueSelector.$in.find(
            placeholder => matcher.documentMatches({ placeholder }).result,
          )
        }
        ...
        if (onlyContainsKeys(valueSelector, ['$gt', '$gte', '$lt', '$lte'])) {
          ...
          if (
            !matcher.documentMatches({ placeholder: middle }).result &&
