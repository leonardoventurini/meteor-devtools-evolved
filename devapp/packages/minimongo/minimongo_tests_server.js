Tinytest.add('minimongo - modifier affects selector', test => {
  ...
  testSelectorPaths(
    {
      $or: [{ x: { $elemMatch: { a: 5 } } }, { y: { $elemMatch: { b: 7 } } }],
    },
    ['x', 'y'],
    '$or and elemMatch',
  )
...
;(() => {
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - literals (structured tests)',
    t => {
      test = t
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - literals (adhoc tests)',
    t => {
      test = t
      ...
      F(
        { 'foo.bar.baz': 1, x: 1 },
        { $unset: { 'foo.bar.baz': 1 }, $set: { x: 1 } },
        'simple unset of the interesting path',
      )
      F(
        { 'foo.bar.baz': 1, x: 1 },
        { $unset: { 'foo.bar': 1 }, $set: { x: 1 } },
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - regexps',
    t => {
      test = t
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - undefined/null',
    t => {
      test = t
      // Nulls / Undefined
      T(
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - literals with arrays',
    t => {
      test = t
      ...
      T(
        { 'a.1.b': 1, x: 1 },
        { $unset: { 'a.1.b': 1 }, $set: { x: 1 } },
        "unset of array element's field with exactly the same index as selector",
      )
      F(
        { 'a.2.b': 1 },
        { $unset: { 'a.1.b': 1 } },
        "unset of array element's field with different index as selector",
      )
      ...
      T(
        { 'foo.bar': null },
        { $set: { 'foo.0.bar': 1, 'foo.1.bar': null } },
        "set array's element's field to null looking for null",
      )
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - set an object literal whose fields are selected',
    t => {
      test = t
      T(
        { 'a.b.c': 1 },
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - $-scalar selectors and simple tests',
    t => {
      test = t
      ...
      T(
        { a: { $lte: 10, $gte: 3, $gt: 3, $lt: 10 }, x: 1 },
        { $set: { x: 1 } },
        'set between x and y - dummy',
      )
      F(
        { a: { $lte: 10, $gte: 13, $gt: 3, $lt: 9 }, x: 1 },
        { $set: { x: 1 } },
      ...
      T(
        { a: { $gt: 9.99999999999999, $lt: 10 }, x: 1 },
        { $set: { x: 1 } },
      ...
      T(
        { a: { $gt: 9.999999999999999, $lt: 10 }, x: 1 },
        { $set: { x: 1 } },
        'very close $gt and $lt',
      )
  ...
  Tinytest.add(
    'minimongo - can selector become true by modifier - $-nonscalar selectors and simple tests',
    t => {
      test = t
